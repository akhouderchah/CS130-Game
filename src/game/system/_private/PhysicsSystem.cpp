...
btBroadphaseInterface *PhysicsSystem::s_pBroadphase = new btDbvtBroadphase();
btCollisionConfiguration *PhysicsSystem::s_pCollisionConfig = new btDefaultCollisionConfiguration();
btDispatcher *PhysicsSystem::s_pDispatcher = new btCollisionDispatcher(s_pCollisionConfig);
btConstraintSolver *PhysicsSystem::s_pSolver = new btSequentialImpulseConstraintSolver;
btDynamicsWorld *PhysicsSystem::s_pWorld = new btDiscreteDynamicsWorld(s_pDispatcher, s_pBroadphase, s_pSolver, s_pCollisionConfig);
...
{
	s_pWorld->setGravity(btVector3(0.0f, -9.8f, 0.0f));
}
...
void PhysicsSystem::Shutdown()
{
	...
	{
		s_pWorld->removeCollisionObject(m_pPhysicsComponents[i]->getBodyStructure()->body);
...
void PhysicsSystem::Tick(deltaTime_t dt)
{
	for(size_t i = 1; i < m_pPhysicsComponents.size(); ++i)
	{
		...
		if (PhysicsComponent::getPauseState())
		{
